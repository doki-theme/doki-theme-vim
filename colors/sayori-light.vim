...
let g:colors_name = 'sayori-light'
...
let s:fg        = g:sayori-light#palette.fg

let s:bglighter = g:sayori-light#palette.bglighter
let s:bglight   = g:sayori-light#palette.bglight
let s:bg        = g:sayori-light#palette.bg
let s:bgdark    = g:sayori-light#palette.bgdark
let s:bgdarker  = g:sayori-light#palette.bgdarker

let s:comment   = g:sayori-light#palette.comment
let s:selection = g:sayori-light#palette.selection
let s:subtle    = g:sayori-light#palette.subtle

let s:cyan      = g:sayori-light#palette.cyan
let s:green     = g:sayori-light#palette.green
let s:orange    = g:sayori-light#palette.orange
let s:pink      = g:sayori-light#palette.pink
let s:purple    = g:sayori-light#palette.purple
let s:red       = g:sayori-light#palette.red
let s:yellow    = g:sayori-light#palette.yellow

let s:none      = ['NONE', 'NONE']

if has('nvim')
  for s:i in range(16)
    let g:terminal_color_{s:i} = g:sayori-light#palette['color_' . s:i]
  endfor
endif
...
if has('terminal')
  ...
  for s:i in range(16)
    call add(g:terminal_ansi_colors, g:sayori-light#palette['color_' . s:i])
  endfor
endif
...
if !exists('g:sayori-light_bold')
  let g:sayori-light_bold = 1
endif

if !exists('g:sayori-light_italic')
  let g:sayori-light_italic = 1
endif

if !exists('g:sayori-light_underline')
  let g:sayori-light_underline = 1
endif

if !exists('g:sayori-light_undercurl') && g:sayori-light_underline != 0
  let g:sayori-light_undercurl = 1
endif

if !exists('g:sayori-light_inverse')
  let g:sayori-light_inverse = 1
endif

if !exists('g:sayori-light_colorterm')
  let g:sayori-light_colorterm = 1
endif
...
let s:attrs = {
      \ 'bold': g:sayori-light_bold == 1 ? 'bold' : 0,
      \ 'italic': g:sayori-light_italic == 1 ? 'italic' : 0,
      \ 'underline': g:sayori-light_underline == 1 ? 'underline' : 0,
      \ 'undercurl': g:sayori-light_undercurl == 1 ? 'undercurl' : 0,
      \ 'inverse': g:sayori-light_inverse == 1 ? 'inverse' : 0,
      \}
...
call s:h('Normal', s:fg, g:sayori-light_colorterm || has('gui_running') ? s:bg : s:none )
